...
describe('Test Pagination', () => {
    ...
    it('Navigate menu through pagination bar', () => {
        ...
        cy.get('[data-cy=paginationDiv]').contains(1).click()
        cy.url().should('include', 'menu?page=1')
        //opacity check
        cy.get('[data-cy=paginationDiv]').contains(1).should('have.css', 'opacity', '1')
        cy.get('[data-cy=paginationDiv]').contains(2).should('have.css', 'opacity', '0.2')
        //go back using back arrow but arrow should be disabled
        cy.get('[data-cy=paginationBack]').should('be.disabled')
        //go to page 2 using pagination box
        cy.get('[data-cy=paginationDiv]').contains(2).click()
        cy.url().should('include', 'menu?page=2')
        cy.get('[data-cy=paginationDiv]').contains(2).should('have.css', 'opacity', '1')
        cy.get('[data-cy=paginationDiv]').contains(1).should('have.css', 'opacity', '0.2')
        cy.get('[data-cy=paginationDiv]').contains(3).should('have.css', 'opacity', '0.2')
        ...
        cy.get('[data-cy=paginationNext]').should('be.disabled')
        //check query is kept
        cy.get('[data-cy=paginationDiv]').contains(1).click()
