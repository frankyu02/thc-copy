...
describe('Test Categories', () => {
    ...
    it('Navigates through menu categories', () => {
        ...
        cy.get('[data-cy=categories]').should('not.contain', 'edibles').should('not.contain', 'flowers').should('not.contain', 'flowers')
        ...
        cy.get('[data-cy=categories]').contains('chocolates').click()
        cy.url().should('eq', 'http://localhost:8000/menu?category=EDIBLES&subcategory=CHOCOLATES')
        cy.get('[data-cy=categories]').contains('gummies').click()
